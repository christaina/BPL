classdef MCMC < BetterHandle
    ...
    methods
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        function [M,accept] = mh_eval_flip_token(this,sid,M,lib,searchPM,verbose)
            %% inspired from mh_eval_spot_token
	    fprintf(1,' trying a flip');
            % score current M
            curr_score = scoreMP(M,lib);
            fast_mode = true
	    Od = SearchMoves(M,lib,verbose,fast_mode);
	    % make 1 flip
	    Od.move_dir_order_rel();
	    % Try all possible flips and find best flips
	    % Od.move_opt_dir_order_rel();
	    % score new M
	    prop_score = scoreMP(Od.M,lib);
            % accept or reject
            accept = mh_accept(prop_score,curr_score);
            if accept
		fprintf(1,' flip accepteed');
                M = Od.M.copy();
	    else
        ...
        function [M,accept] = mh_eval_split_merge_token(this,sid,M,lib,verbose)
